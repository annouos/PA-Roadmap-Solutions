...
void fill_sum_ray(int ray[3][4], int ray_sum[4], short row, short col_number)
{
    for (short j = 0; j < col_number; j++)
    {
        ray_sum[j] = sum_ray(ray, row, j);
    }
}
...
void print_sum(int ray_sum[4], short col)
{
    cout << "\nThe sum of matrix is: \n";
    for (short i = 0; i < col; i++)
    {
        cout << "\nCol in " << i+1 << " sum = " << ray_sum[i] << endl;
    }
}
