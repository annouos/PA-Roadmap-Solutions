...
bool switch_to_delete(string wanted, vector <info> &favorita)
{
    for (info &x : favorita)
    {
        if (x.acount == wanted)
...
bool delete_account(string wanted, vector <info> &favorito)
{
    info self;
    char answer = 'Y';
    ...
    if (search_account(wanted, favorito, self))
    {
        ...
        if (toupper(answer) == 'Y')
        {
            switch_to_delete(wanted, favorito);
...
{   
    ...
    delete_account(read_string("For whome account do you want to delete ? "), favorite);
    return 0;
}
